...
export function normaliseTool(raw: RawTool): NormalisedTool {
...
export async function normaliseReco(data: RawRecoData, currentSlug: string): Promise<NormalisedReco> {
  const tools = (data.tools || []).map(normaliseTool);
  ...
  if (data.sections && data.sections.length) {
    ...
    rawSections = [{ id: 'all', romanNumeral: 'I.', title: data.title, label: 'All entries' }];
  }
